...
######STEP 4: LONGITUDNAL CFA WEAK INVARIANCE 
...
weak_fit1 <- cfa(weak1, data = cbclDF1, mimic = "mplus")
summary(weak_fit1, fit.measures = TRUE, standardized=TRUE)
#factor.scores(cbclDF1, weak_fit1)

#create a model and CFA for baseline and two year WEAK INVARAINCE 
weak2<- '
#Define latent 
   baseFactor1=~
NA*baseAttacksThreatens
...
weak_fit2 <- cfa(weak2, data = cbclDF2, mimic = "mplus")
summary(weak_fit2, fit.measures = TRUE, standardized=TRUE)
#factor.scores(cbclDF2, weak_fit2)

#Model diagram for baseline-one year
#semPaths(weak_fit1, what="est", sizeLat = 7, sizeMan = 7, edge.label.cex = .75)

#Model diagram for baseline-two year
#semPaths(weak_fit2, what="est", sizeLat = 7, sizeMan = 7, edge.label.cex = .75)
