...
######STEP 4: LONGITUDNAL CFA STRICT INVARIANCE 
...
strict_fit1 <- cfa(strict1, data = cbclDF1, mimic = "mplus")
summary(strict_fit1, fit.measures = TRUE, standardized=TRUE)
#factor.scores(cbclDF1, strict_fit1)

#create a model and CFA for baseline and two year STRICT INVARAINCE 
strict2<- '
#Define latent 
   baseFactor1=~
NA*baseAttacksThreatens
...
strict_fit2 <- cfa(strict2, data = cbclDF2, mimic = "mplus")
summary(strict_fit2, fit.measures = TRUE, standardized=TRUE)
#factor.scores(cbclDF2, strict_fit2)

#Model diagram for baseline-one year
#semPaths(strict_fit1, what="est", sizeLat = 7, sizeMan = 7, edge.label.cex = .75)

#Model diagram for baseline-two year
#semPaths(strict_fit2, what="est", sizeLat = 7, sizeMan = 7, edge.label.cex = .75)
