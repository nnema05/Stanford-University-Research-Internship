...
##### abcd_acs_raked_propensity
#FILES: ACS2011_15_I.Rds ,AND
#Demo.xlsx (not provided) - This is the baseline demographic data from ABCD study. If you have the access to ABCD report data dump, it is in the "ABCD 2 release Mental Health", "P Demographic" sheet.
#AND ACS.xlsx (not provided) - This is a second data table from the ABCD study. If you have the access to ABCD report data dump, it is in the "ABCD 2 release Mental Health", "ACS" sheet. 
...
#DEMO 
ysuip <- read.delim("/PATH/abcd_ysuip01_d.txt")
pdem02 <- read.delim("/PATH/pdem02_d.txt")
abcd_lt01 <- read.delim("/PATH/abcd_lt01_d.txt")
dat.demo <- merge(ysuip, pdem02, by = c("subjectkey"))
dat.demo <- merge(dat.demo, abcd_lt01, by = c("subjectkey", "eventname"))
dat.demo=dat.demo[dat.demo$eventname == "baseline_year_1_arm_1",]
dim(dat.demo)

#ACS 
dat.acs <- read.delim("/PATH/acspsw03_d.txt")
dat.acs <- merge(dat.acs, abcd_lt01)
dat.acs = dat.acs[dat.acs$eventname == "baseline_year_1_arm_1",]
nrow(dat.acs) 

dat.2011_15=readRDS("/PATH/ACS2011_15_I.Rds")
...
newdemo=dat.demo[,c("subjectkey","site_name",selected.vars)]
nrow(newdemo)  
...
acs_s <- acs[,c("subjectkey", "rel_family_id", "rel_group_id", "rel_relationship", "rel_same_sex", "site_name")]
acs_s[,2:5]=sapply(acs_s[,2:5],as.numeric)
names(acs_s) 
summary(acs_s)

#ALLMULT VARIABLES (MULTIPLE BIRTHS)
acs_s$allmult <- 0 
acs_s$allmult [acs_s$rel_relationship %in% c(2,3)] <- 1 
table(acs_s$allmult) 

#TWIN SITE VARIBALE: note corrected to VCU assignment as twin site (SH) 
acs_s$sitetwin <- 0 
...
############################################################################
#####STEP 4: merge temp1 saved above and acs_s into tmpmerge

# merge temp1 saved above and acs_s into tmpmerge 
tmpmerge <- merge(newdemo, acs_s, by="subjectkey") 
summary(tmpmerge) 
names(tmpmerge)
...
# use names to select 
tmpimp <- tmpmerge[c("subjectkey", "age", "sex", "allmult", "famtype", "faminc", "sitetwin",
...
abcd_nov18_i_s <- abcd_nov18_i[c("PWGTP", "age", "sex", "region", "race_eth","faminc", "famtype", 
                                 "hhsize","allmult","subjectkey", "site_name", "fesabcd", "acsflag", "demo_prtnr_empl_v2b", "rel_relationship", "sitetwin")] 
...
write.csv(abcdsub, file = "/PATH//Jan_2019abcd_R.csv")
...
####ADD THIS FINAL PHENOTYPE 
#rpwgtmeth1 is acs_raked_propensity_score
library(tidyverse)
