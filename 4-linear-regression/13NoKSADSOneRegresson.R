...
## Then a linear regression model is run between these phenotype and THE DIFFERENCE BEWTEEN ONE YEAR AND BASELINE p-factor and all 5 factor scores
...
diffScores = read.csv("/PATH/DiffOneScores.csv")
dat = read.csv("/PATH/finalphenotypes_all.csv", na.strings = c("NA", "", " "))
print("READ DATA AND DIFF SCORES ")

#For creating new phenotype file with ksad and site_id
#phenotype <- read.csv("/PATH/phenotypes_all.csv", na.strings = c("NA", "", " "))
#ksad <- read.csv("/PATH/fullKsadsPhenotype.csv")
#phenotype <- subset(phenotype, select = -c(Any_DepDx:Any_ODD))
#merge ksad and phenotype
#finalphenotype <- left_join(phenotype, ksad, by = c("subjectkey" = "ksad.subjectkey", "eventname" = "ksad.eventname"))


#siteId <- read.delim("/PATH//abcd_lt01_d.txt")
#siteId <- siteId %>% 
  #select(subjectkey, eventname, site_id_l)
#finalphenotype <- left_join(finalphenotype, siteId, by = c("subjectkey", "eventname"))
#finalphenotype <- finalphenotype %>% 
#rename(Any_DepDx  = Any_DepDx.Any_DepDx
#         , ANY_SUISH = ANY_SUISH.ANY_SUISH
#         , ANY_GAD = ANY_GAD.ANY_GAD
#         , Any_SocAnx = Any_SocAnx.Any_SocAnx
#         , Any_SepAnx = Any_SepAnx.Any_SepAnx
#         , Any_Phobia = Any_Phobia.Any_Phobia
#         , Any_PanicAgorDx = Any_PanicAgorDx.Any_PanicAgorDx
#         , Any_BipolarDx = Any_BipolarDx.Any_BipolarDx
#         , Any_ADHD = Any_ADHD.Any_ADHD
#         , Any_CD = Any_CD.Any_CD
#         , Any_ODD = Any_ODD.Any_ODD)
#write.csv(finalphenotype, "/PATH//finalphenotypes_all.csv")
...
print("FILTER ON EVENT NAME AND MERGE")
...
print("FIX RACE AND ETHNCITY FROM 1 TO WHITE")
...
print("FIX MARITAL STATUS FROM 1 TO YES")
...
print("FIX HOUSEHOLD INCOME FROM 1 to <50K")
...
print("controls and nesting structure for mixed-effects models")
...
#covariates = cbind(covariates,  covariateBasePfactor = dat$basePFactor)
print("DUMMY CODING COVARAITES")
dat = cbind(dat, covariates)
print("BIND DATA AND COVARIATES")
...
print("KSAD DIAGNOSES INTO FACTORS")
...
print("declare the phenotype set")

######STEP 4: DEPENDENTS 
dependents = c('DiffOnePFactor', 'DiffOneFactor1', 'DiffOneFactor2', 
               'DiffOneFactor3', 'DiffOneFactor4', 'DiffOneFactor5')
print("declare dependents ")
...
######STEP 6 :SAVE AS CSV
write.csv(full_res, file='13OneNoKSADSRes.csv', quote=FALSE, row.names=FALSE)
print("SAVE RESULSTS IN A CSV")