...
## Then a linear regression model is run between these phenotype and THE DIFFERENCE BEWTEEN TWO YEAR AND BASELINE p-factor and all 5 factor scores
...
for (dependent in dependents) {
  ...
  for (phenotype in phenotypes) {
    covariateDepFact <- case_when(
      dependent == 'DiffTwoPFactor' ~ "basePFactor",
      dependent == 'DiffTwoFactor1' ~ "baseFactor1", 
      dependent == 'DiffTwoFactor2' ~ "baseFactor2",
      dependent == 'DiffTwoFactor3' ~ "baseFactor3",
      dependent == 'DiffTwoFactor4' ~ "baseFactor4")
...
######STEP 6 :SAVE AS CSV
write.csv(full_res, file='13TwoNoKSADSRes.csv', quote=FALSE, row.names=FALSE)