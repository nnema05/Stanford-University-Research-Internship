...
finalphenotypes <- read.csv("/PATH/finalphenotypes_all.csv", na.strings = c("NA", "", " "))
acsraked <- read.csv("/PATH/Jan_2019abcd_R.csv")
acsraked <- acsraked %>%
  select(subjectkey, eventname, rpwgtmeth1)
finalphenotype <- left_join(finalphenotype, acsraked, by = c("subjectkey", "eventname"))
...
######STEP 4: DEPENDENTS 
dependents = c('basePFactor', 'baseFactor1', 'baseFactor2', 
               'baseFactor3', 'baseFactor4', 'baseFactor5')
...
######STEP 6 :SAVE AS CSV
write.csv(full_res, file='13BaseNoKSADSRes.csv', quote=FALSE, row.names=FALSE)
