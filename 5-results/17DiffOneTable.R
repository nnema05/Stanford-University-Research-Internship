...
## This program, using SQL, works to create a final table that will hold all p-values from regression called 17DiffOneRegression.csv
## This p-values showing the association between phenotypes and Difference in ONE year and baseline scores 
...
OneNewRes <- read.csv("/PATH/16OneNewRes.csv")
OneNoKSADSRes <- read.csv("/PATH/13OneNoKSADSRes.csv")
OneKSADSRes <- read.csv("/PATH/14OneKSADSRes.csv")
OneDemoRes <- read.csv("/PATH/15OneDemoRes.csv")
...
,CASE WHEN FDR_std_pvalue < 0.001  and std_beta >= 0.2  THEN std_beta_s||'**'||' ('||std_lower_ci||'-'||std_upper_ci||')' 
 ...
 ELSE std_beta_s||' ('||std_lower_ci||'-'||std_upper_ci||')' END DiffOnep
from (
SELECT 
...
FROM OneNewRes 
WHERE dependent in ('DiffOnePFactor'))")
...
,CASE WHEN FDR_std_pvalue < 0.001  and std_beta >= 0.2  THEN std_beta_s||'**'||' ('||std_lower_ci||'-'||std_upper_ci||')' 
      WHEN FDR_std_pvalue < 0.001 OR std_beta >= 0.2  THEN std_beta_s||'*'||' ('||std_lower_ci||'-'||std_upper_ci||')'
 ELSE std_beta_s||' ('||std_lower_ci||'-'||std_upper_ci||')' END DiffOneEXT
from (
SELECT 
  CASE WHEN phenotype LIKE 'gender%' THEN 'Sex'
            WHEN phenotype LIKE 'race%' THEN 'Race/Ethnicity'
  ...
  ,CAST(ROUND(std_upper_ci,2) AS STRING) as std_upper_ci  
FROM OneNewRes 
WHERE dependent in ('DiffOneFactor1'))")
...
,CASE WHEN FDR_std_pvalue < 0.001  and std_beta >= 0.2  THEN std_beta_s||'**'||' ('||std_lower_ci||'-'||std_upper_ci||')' 
      WHEN FDR_std_pvalue < 0.001 OR std_beta >= 0.2  THEN std_beta_s||'*'||' ('||std_lower_ci||'-'||std_upper_ci||')'
 ELSE std_beta_s||' ('||std_lower_ci||'-'||std_upper_ci||')' END DiffOneINT
from (
SELECT 
  CASE WHEN phenotype LIKE 'gender%' THEN 'Sex'
            WHEN phenotype LIKE 'race%' THEN 'Race/Ethnicity'
  ...
  ,CAST(ROUND(std_upper_ci,2) AS STRING) as std_upper_ci 
FROM OneNewRes 
WHERE dependent in ('DiffOneFactor2'))")

NEWcompare <- merge(pfact, fact1, by = c("phenotype", "header"))
NEWcompare <- merge(NEWcompare, fact2, by = c("phenotype", "header"))
NEWcompare <- NEWcompare %>% select(header, phenotype, DiffOnep, DiffOneINT, DiffOneEXT)
NEWcompare <- NEWcompare %>% arrange(header)
write.csv(NEWcompare, "17NEWDiffOne.csv")
            

#, CASE WHEN dependent=='onePFactor' THEN 'DiffOnePFactor'
#  WHEN dependent=='oneFactor1' THEN 'DiffOneEXT'
#  WHEN dependent=='oneFactor2' THEN 'DiffOneINT' END DiffOnep
...
DEMOcompare <- DEMOcompare %>% select(header, phenotype, DiffOnep, DiffOneINT, DiffOneEXT)
DEMOcompare <- DEMOcompare %>% arrange(header)
write.csv(DEMOcompare, "17DEMODiffOne.csv")
...
KSADScompare <- KSADScompare %>% select(header, phenotype, DiffOnep, DiffOneINT, DiffOneEXT)
KSADScompare <- KSADScompare %>% arrange(header)
write.csv(KSADScompare, "17KSADSDiffOne.csv")
...
NOKSADScompare <- NOKSADScompare %>% select(header, phenotype, DiffOnep, DiffOneINT, DiffOneEXT)
NOKSADScompare <- NOKSADScompare %>% arrange(header)
write.csv(NOKSADScompare, "17NOKSADSDiffOne.csv")

#To combine 
#cat 17NEWDiffOne.csv 17DEMODiffOne.csv 17KSADSDiffOne.csv 17NOKSADSDiffOne.csv > 17DiffOneRegression.csv
#remove extra headers
#vi 17DiffOneRegression.csv, /header, dd
