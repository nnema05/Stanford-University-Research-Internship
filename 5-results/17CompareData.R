...
## This program, using SQL, works to create a final table that will hold all p-values from regression called 17BaselineRegression_nocovariate.csv
## This p-values showing the association between phenotype and baseline scores!
...
BaseNewRes <- read.csv("/PATH/16BaseNewRes.csv")
BaseNoKSADSRes <- read.csv("/PATH/13BaseNoKSADSRes.csv")
BaseKSADSRes <- read.csv("/PATH/14BaseKSADSRes.csv")
BaseDemoRes <- read.csv("/PATH/15BaseDemoRes.csv")
...
,CASE WHEN FDR_std_pvalue < 0.001  and std_beta >= 0.2  THEN std_beta_s||'**'||' ('||std_lower_ci||'-'||std_upper_ci||')' 
 ...
 ELSE std_beta_s||' ('||std_lower_ci||'-'||std_upper_ci||')' END p 
from (
SELECT 
...
FROM BaseNewRes 
WHERE dependent in ('basePFactor'))")
...
,CASE WHEN FDR_std_pvalue < 0.001  and std_beta >= 0.2  THEN std_beta_s||'**'||' ('||std_lower_ci||'-'||std_upper_ci||')' 
      WHEN FDR_std_pvalue < 0.001 OR std_beta >= 0.2  THEN std_beta_s||'*'||' ('||std_lower_ci||'-'||std_upper_ci||')'
 ELSE std_beta_s||' ('||std_lower_ci||'-'||std_upper_ci||')' END EXT
from (
SELECT 
  CASE WHEN phenotype LIKE 'gender%' THEN 'Sex'
            WHEN phenotype LIKE 'race%' THEN 'Race/Ethnicity'
  ...
  ,CAST(ROUND(std_upper_ci,2) AS STRING) as std_upper_ci  
FROM BaseNewRes 
WHERE dependent in ('baseFactor1'))")
...
,CASE WHEN FDR_std_pvalue < 0.001  and std_beta >= 0.2  THEN std_beta_s||'**'||' ('||std_lower_ci||'-'||std_upper_ci||')' 
      WHEN FDR_std_pvalue < 0.001 OR std_beta >= 0.2  THEN std_beta_s||'*'||' ('||std_lower_ci||'-'||std_upper_ci||')'
 ELSE std_beta_s||' ('||std_lower_ci||'-'||std_upper_ci||')' END INT
from (
SELECT 
  CASE WHEN phenotype LIKE 'gender%' THEN 'Sex'
            WHEN phenotype LIKE 'race%' THEN 'Race/Ethnicity'
  ...
  ,CAST(ROUND(std_upper_ci,2) AS STRING) as std_upper_ci 
FROM BaseNewRes 
WHERE dependent in ('baseFactor2'))")

NEWcompare <- merge(pfact, fact1, by = c("phenotype", "header"))
NEWcompare <- merge(NEWcompare, fact2, by = c("phenotype", "header"))
NEWcompare <- NEWcompare %>% select(header, phenotype, p, INT, EXT)
NEWcompare <- NEWcompare %>% arrange(header)
write.csv(NEWcompare, "17NEWcompare.csv")
            

#, CASE WHEN dependent=='basePFactor' THEN 'PFactor'
#  WHEN dependent=='baseFactor1' THEN 'EXT'
#  WHEN dependent=='baseFactor2' THEN 'INT' END p
...
DEMOcompare <- DEMOcompare %>% select(header, phenotype, p, INT, EXT)
DEMOcompare <- DEMOcompare %>% arrange(header)
write.csv(DEMOcompare, "17DEMOcompare.csv")
...
KSADScompare <- KSADScompare %>% select(header, phenotype, p, INT, EXT)
KSADScompare <- KSADScompare %>% arrange(header)
write.csv(KSADScompare, "17KSADScompare.csv")
...
NOKSADScompare <- NOKSADScompare %>% select(header, phenotype, p, INT, EXT)
NOKSADScompare <- NOKSADScompare %>% arrange(header)
write.csv(NOKSADScompare, "17NOKSADScompare.csv")

#To combine 
#cat 17NEWcompare.csv 17DEMOcompare.csv 17KSADScompare.csv 17NOKSADScompare.csv > 17BaselineRegression_nocovariate.csv
